...
export function AppDetail({ app, onClose }: AppDetailProps) {
  ...
  const backElement = isOverlay ? (
    <button
      onClick={onClose}
      className="flex items-center gap-2 text-text-primary bg-transparent border-none cursor-pointer hover:opacity-70 transition-opacity p-0"
    >
  ...
  const detail = (
    <div className={`${isOverlay ? "h-full" : "h-dvh"} bg-bg text-text-primary font-sans flex flex-col overflow-hidden`}>
      ...
      <main className="relative z-10 flex-1 min-h-0 flex items-center justify-center p-4 sm:p-6">
        <motion.div
          initial={isOverlay ? false : { opacity: 0, y: 30 }}
        ...
        >
          ...
          <div className="w-full md:w-2/5 p-6 sm:p-8 flex flex-col items-center justify-center bg-surface-container-low/50 relative overflow-hidden">
            <div className="relative z-10 flex flex-col items-center">
              {/* Screenshot carousel or large icon */}
              {app.screenshots && app.screenshots.length > 0 ? (
                ...
                <motion.div
                  initial={isOverlay ? false : { scale: 0.8, opacity: 0 }}
                  animate={{ scale: 1, opacity: 1 }}
                  transition={{ delay: isOverlay ? 0 : 0.2, duration: 0.5, ease: [0.34, 1.56, 0.64, 1] }}
                  className="mb-5"
                >
                  <div className="w-32 h-32 sm:w-36 sm:h-36 bg-surface-container-lowest rounded-[2rem] shadow-xl flex items-center justify-center">
                    <AppIcon
                      ...
                      layoutId={isOverlay ? `icon-${app.id}` : undefined}
                    />
                  </div>
                </motion.div>
              )}
          ...
          <div className="w-full md:w-3/5 p-6 sm:p-8 bg-surface-container-lowest flex flex-col justify-between overflow-y-auto">
            <div>
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: isOverlay ? 0.1 : 0.3 }}
              ...
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: isOverlay ? 0.15 : 0.35 }}
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: isOverlay ? 0.2 : 0.45 }}
              ...
              {app.features && app.features.length > 0 && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ delay: isOverlay ? 0.25 : 0.5 }}
                  className="mb-6"
                >
                  <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-2">
                    Features
            ...
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: isOverlay ? 0.3 : 0.6 }}
